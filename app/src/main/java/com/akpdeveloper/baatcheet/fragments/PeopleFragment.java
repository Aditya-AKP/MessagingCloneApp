...
//        getDB();
    ...
    private void fetchingTheContactsFromFirebase(List<UserModel> userModelList,List<String> phoneNoList,int i){
        ...
        if(i+30>=phoneNoList.size()){
            FireBaseClass.allUsersCollectionReference().whereIn("number",phoneNoList.subList(i, phoneNoList.size()-1)).get().addOnCompleteListener(task -> {
